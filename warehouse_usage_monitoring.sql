...
def run(session, alert_threshold_low:float, alert_threshold_high:float, warehouse_names:list, email_addresses:list):
    ...
    if not warehouse_names:
        warhouse_names = summ['WAREHOUSE_NAME'].tolist()
        
    alert = summ.loc[summ['WAREHOUSE_NAME'].isin(warehouse_names)]
    alert = summ.loc[(summ['CREDITS_USED_VAR_TO_TREND'] < alert_threshold_low) | (summ['CREDITS_USED_VAR_TO_TREND'] > alert_threshold_high)]
...
-- test 
call warehouse_usage_monitoring(-0.5, 0.5, [], ['your.email@snowflake.com']);

-- schedule a run at 5 minutes after every hour
create or replace task usage_monitoring 
    warehouse='XSMALL'
    schedule= 'USING CRON 5 * * * * America/Chicago'
    as call warehouse_usage_monitoring(-0.5, 0.5, [], ['jeremy.griffith@snowflake.com']);
